...
<h1 class="mb-5">Post Category : {{ $category }}</h1>
    ...
    <article class="mb-5 border-bottom pb-4">
        ...
        <p>By <a class="text-decoration-none" href="/author/{{ $post->user->name }}"> {{ $post->user->name }} </a> in <a class="text-decoration-none" href="/categories/{{ $post->category->slug }}">{{ $post->category->name }}</a></p>
        <p>{!! $post['excerpt'] !!}</p>
